...
CertLogic.prototype.checkCertWithPassword = function(options, callback) {
  ...
  var uri = util.format(
    '%s/v1/organizations/%s/environments/%s/keyvaluemaps/%s',
    this.managementUri,
    options.org,
    options.env,
    this.vaultName
...
CertLogic.prototype.checkPrivateCert = function(options, callback) {
  var uri = util.format(
    '%s/v1/organizations/%s/environments/%s/keyvaluemaps/%s/entries/private_key',
    this.managementUri,
    options.org,
    options.env,
    this.vaultName
      ...
      async.series(
        [
          ...
          function(cb) {
            ...
            createVault(
              generateCredentialsObject(options),
              managementUri,
              options.org,
              options.env,
              vaultName,
            ...
            request(
              {
                uri: regionUrl,
                auth: {
                  // switch authorization to use the key/secret we just uploaded
                  username: key,
                  password: secret
                },
                json: true
...
function createVault(credentials, managementUri, organization, environment, vaultName, entries, cb) {
  ...
  var uri = util.format(
    '%s/v1/organizations/%s/environments/%s/keyvaluemaps',
    managementUri,
    organization,
    environment
  );
